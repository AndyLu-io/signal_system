...
# ─── 账户基础参数 ─────────────────────────────────────────────────────────────
ACCOUNT_NET_VALUE = 1_000_000  # 账户净值（元），每月手动更新
...
STOCK_UNIVERSE = {
    ...
    "600989": {
        ...
        "theme": "煤化工/新能源氢能",
        "pool": "candidate", "cluster": "commodity",
        "f_policy": 50,      "f_earnings": 62,
        "signal_3d": "★★☆",
        "fund_delta": 69,    "fund_pct": 84.1,
        "nb_delta": 0.0,     "q1_yoy": 0,
        "etf_overlap": ["159870"],
        "discovered": "2026Q2",
        "note": "煤制烯烃龙头+绿氢布局；化工ETF权重4.28%，低估值高分红，政策受益于能源安全主线",
    },
}
...
# ─── 攻守切换 · 防御轮动池 ────────────────────────────────────────────────────
# 情绪过热（HOT / OVERHEATED）或机制恶化时推荐切换至此池
# defense_rank: 0=避险（类现金/债券），1=极防御（公用事业），2=高股息蓝筹，3=弱防御消费
# 选入标准：稀缺性（垄断牌照/不可替代资源）+ 下跌Beta<0.5 + 抗跌日占比>35%
# 切换节奏：R1仅5%防御 → R2升至15% → R3升至25% → R4升至50%+（rank0占半）
#
# ── 大盘急跌日(沪深300跌>=1%)资金流向实证（3年/80次急跌）─────────────────────
# 避险型（当天跌<0.7%）：国债ETF(+0.05%) > 中国神华(+0.16%) > 宁沪高速(-0.19%)
#                       > 银行ETF(-0.80%) > 红利ETF(-0.91%) > 煤炭ETF(-1.21%)
# 重灾区（当天跌>2.0%）：有色(-2.55%) > 卫星(-2.81%) > 通信(-2.56%)
#                       > 半导体(-2.51%) > 芯片(-2.41%) > 创业板(-2.40%)
# 结论：急跌时资金流向 国债>神华>高速>银行>红利>电力，远离科技/有色/军工
DEFENSIVE_ROTATION_POOL = {
    # ── Rank 0: 避险（零风险/负相关，R4首选）─────────────────────────────────
    "511880": {"name": "银华日利ETF",   "defense_rank": 0, "note": "货币基金ETF，日度申赎，年化1.5-2%"},
    "511010": {"name": "国债ETF",       "defense_rank": 0, "note": "10年国债，急跌日+0.05%正收益率73%，与A股相关性-0.27"},
    "511030": {"name": "同业存单AAA",   "defense_rank": 0, "note": "AAA同业存单，比货基高0.5%，流动性佳"},

    # ── Rank 0 跨境对冲（与A股低相关0.20-0.25，熊市期间正收益）───────────────
    # 实证：23-24危机期间 纳指+19.2%/标普+14.9%/日经+14.7%，而A股-14%
    # 注意：港股ETF相关性0.6-0.7，不能对冲！恒生科技急跌日-2.5%比A股还惨
    "513500": {"name": "标普500ETF",    "defense_rank": 0, "note": "急跌日-0.44%正收益48%，相关性仅0.20，A股熊市期均+11.2%"},
    "513100": {"name": "纳指ETF",       "defense_rank": 0, "note": "急跌日-0.60%正收益43%，相关性0.24，23-24危机+19.2%"},
    "513520": {"name": "日经ETF",       "defense_rank": 0, "note": "急跌日-0.65%，相关性0.25，4段熊市均+1.7%"},

    # ── Rank 1: 极防御（垄断公用事业，急跌日跌<0.7%）──────────────────────────
    "600900": {"name": "长江电力",      "defense_rank": 1, "note": "水电龙头，3年急跌日-0.33%上涨41%，周月线多头"},
    "600025": {"name": "华能水电",      "defense_rank": 1, "note": "水电第二极，3年急跌日-0.50%上涨34%，周线翻转中"},
    "600941": {"name": "中国移动",      "defense_rank": 1, "note": "电信牌照垄断（仅3张），3年急跌日-0.57%上涨35%，周线多头"},
    "600377": {"name": "宁沪高速",      "defense_rank": 1, "note": "高速特许经营权，3年急跌日仅-0.19%上涨42%，周月线多头"},
    "600886": {"name": "国投电力",      "defense_rank": 1, "note": "水电+火电，3年急跌日-0.24%上涨39%，月线翻转中"},
    "000429": {"name": "粤高速A",      "defense_rank": 1, "note": "高速特许经营，4段熊市均+7.8%，周月线多头"},

    # ── Rank 2: 高股息蓝筹 + 周月线走坏降级品种 ──────────────────────────────────
    "512800": {"name": "银行ETF",       "defense_rank": 2, "note": "⚠️周月线走坏(空头+死叉)，急跌日-0.21%仍抗跌，等翻转后回Rank1"},
    "515450": {"name": "红利低波50ETF", "defense_rank": 2, "note": "⚠️周月线走坏(空头+死叉)，急跌日-0.51%，等翻转后回Rank1"},
    "510880": {"name": "红利ETF",       "defense_rank": 2, "note": "⚠️周月线走坏(空头+死叉)，急跌日-0.50%，等翻转后回Rank1"},
    "601088": {"name": "中国神华",      "defense_rank": 2, "note": "⚠️周月线走坏(跌破MA10+死叉)，急跌日+0.16%仍反向，等翻转后回Rank1"},
    "601939": {"name": "建设银行",      "defense_rank": 2, "note": "四大行，年化+12.4%回撤-13%夏普0.61，周月线多头"},
    "003816": {"name": "中广核A",       "defense_rank": 2, "note": "核电牌照（仅3家），偏弱但周MACD金叉，观察"},
    "601006": {"name": "大秦铁路",      "defense_rank": 2, "note": "煤运铁路垄断，偏弱但周MACD金叉，观察"},

    # ── Rank 3: 弱防御/走坏降级品种 ──────────────────────────────────────────────
    "601857": {"name": "中国石油",      "defense_rank": 3, "note": "⚠️周月线走坏(空头+死叉)，油气垄断Beta=-0.16，等翻转后回Rank2"},
    "601398": {"name": "工商银行",      "defense_rank": 3, "note": "⚠️周月线走坏(空头+死叉)，6%股息率，等翻转后回Rank2"},
    "601288": {"name": "农业银行",      "defense_rank": 3, "note": "⚠️周月线空头排列，3年急跌日-0.28%仍优秀，等翻转后回Rank2"},
    "600887": {"name": "伊利股份",      "defense_rank": 3, "note": "⚠️月线空头但周MACD金叉，刚需消费观察"},
    "000651": {"name": "格力电器",      "defense_rank": 3, "note": "偏弱但周MACD金叉，7%股息率，观察"},
    "000568": {"name": "泸州老窖",      "defense_rank": 3, "note": "⚠️周月线空头排列+死叉，白酒下行周期，最低优先级"},
}
